...
class HumanAgent(AutonomousAgent):
    ...
    def run_step(self, input_data):
        ...
        grey = input_data['Grayscale'].get(active_camera, None)
        semantic = input_data['Semantic'].get(active_camera, None)
        ...
        if grey is not None and self.taking_photo:
            now = datetime.datetime.now()
            cv2.imwrite(f"../rockdata/grey/" + now.strftime("%Y-%m-%d %H:%M:%S") + ".png", grey)
            cv2.imwrite(f"../rockdata/semantic/" + now.strftime("%Y-%m-%d %H:%M:%S") + ".png", semantic)
            self.photos_taken += 1
            print("Photo Taken")
            self.taking_photo = False
