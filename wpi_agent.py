...
from point_cloud.generate_pcd import generate_pcd
...
class HumanInterface(object):
    ...
    def __init__(self, width, height, controller, scale):
        ...
        # Video Writers for FrontLeft and FrontRight cameras
        self.front_left_writer = cv2.VideoWriter('/home/vishwas/LunarAutonomyChallenge/agents/videos/front_left.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 20, (width, height))
        self.front_right_writer = cv2.VideoWriter('/home/vishwas/LunarAutonomyChallenge/agents/videos/front_right.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 20, (width, height))
        self.left_img_folder = "/home/vishwas/LunarAutonomyChallenge/agents/left_images/"
        self.right_img_folder = "/home/vishwas/LunarAutonomyChallenge/agents/right_images/"
        self.prev_time = 0


    def run_interface(self, sensor_data, semantic_data, vehicle_data, imu_data, refresh=True, front_left_data=None, front_right_data=None):
        """
        Run the GUI
        """
        frame_name = str(time.time())
        if front_left_data is not None:
            front_left_frame = front_left_data
            front_left_frame = np.stack([front_left_frame, front_left_frame, front_left_frame], axis = -1)
            front_left_frame = front_left_frame.swapaxes(0, 1)
            # front_left_frame = cv2.cvtColor(front_left_frame, cv2.COLOR_BGRA2BGR)
            # self.front_left_writer.write(np.swapaxes(front_left_frame, 0, 1))
            # cv2.imshow('FrontLeft Camera', np.swapaxes(front_left_frame, 0, 1))
            # cv2.waitKey(1)

        if front_right_data is not None:
            front_right_frame = front_right_data
            front_right_frame = np.stack([front_right_frame, front_right_frame, front_right_frame], axis = -1)
            front_right_frame = front_right_frame.swapaxes(0, 1)
        
        if front_left_data is not None and front_right_data is not None and time.time() - self.prev_time > 3:
            # cv2.imwrite(self.left_img_folder + frame_name + ".png", np.swapaxes(front_left_frame, 0, 1))
            # cv2.imwrite(self.right_img_folder + frame_name + ".png", np.swapaxes(front_right_frame, 0, 1))
            self.prev_time = time.time()
            
            # self.front_right_writer.write(np.swapaxes(front_right_frame, 0, 1))
        # Process camera data
        image = None
        ...
        # cv2.imshow('Front Camera', np.swapaxes(image, 0, 1))
        # cv2.waitKey(1)
        ...
        # image = pygame.surfarray.array3d(self._surface)
    ...
    def quit(self):
        """Quit the application and release video writers"""
        self.front_left_writer.release()
        self.front_right_writer.release()
        pygame.quit()
...
class Mapping(object):
    ...
    def __init__(self):
        self.pcd_model = generate_pcd()
        self.global_pcd = None
        self.prev_time = 0
    
    def get_local_pcd(self, front_left_data, front_right_data):
        if front_left_data is not None:
            front_left_frame = front_left_data
            front_left_frame = np.stack([front_left_frame, front_left_frame, front_left_frame], axis = -1)
            front_left_frame = front_left_frame.swapaxes(0, 1)
        ...
        if front_left_data is not None and front_right_data is not None and time.time() - self.prev_time > 3:
            left_image = np.swapaxes(front_left_frame, 0, 1)
            right_image = np.swapaxes(front_right_frame, 0, 1)
            self.prev_time = time.time()
        return self.pcd_model.get_pcd(left_image, right_image)
    
    def get_global_pcd(self, local_pcd):
...
class HumanAgent(AutonomousAgent):
    ...
    def run_step(self, input_data):
        ...
        # cv2.imshow('Front Camera', input_data['Grayscale'].get(active_camera, None))
        front_left_data = input_data['Grayscale'].get(carla.SensorPosition.FrontLeft, None)
        front_right_data = input_data['Grayscale'].get(carla.SensorPosition.FrontRight, None)
        ...
        self._hic.run_interface(
            ...
            front_left_data=front_left_data,
            front_right_data=front_right_data
        )
        pcd = self._mapping.get_local_pcd(front_left_data, front_right_data)
        return control
